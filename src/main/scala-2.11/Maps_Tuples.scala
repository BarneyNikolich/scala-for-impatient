...
object Maps_Tuples extends App {
  ...
  def readFileJavaIO = {
      ...
      val wordCount = scala.collection.mutable.Map[String, Int]() withDefault (_ => 0) //Originally maps all inputs to 0.
...
//  4 - Print sorted list
  ...
  def randArray(n: Int): Array[Int] = {
    val randa = new Array[Int](n)
    for(i <- 0 until randa.length) randa(i) = Random.nextInt(n)
    randa
  }
